...
for df in datafiles:
    if df.endswith('.csv'):
        print '* running dsim with model bsrf, output DHbO2 and input ' + df
        subprocess.call(['/usr/bin/env', 'python', 'dsim.py', 'bsrf_DHbO2.dsimjob', 'scratch/hx/' + df])
...
for df in datafiles:
    if df.endswith('.csv'):
        print '* running dsim with model bsrf_BS, output DHbO2 and input ' + df
        subprocess.call(['/usr/bin/env', 'python', 'dsim.py', 'bsrf_BS_DHbO2.dsimjob', 'scratch/hx/' + df])
